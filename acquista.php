        ...
        <?php
            ...
            session_start();
                if(isset ($_SESSION ['user'])){
                    ...
                    $sql="SELECT prodotto.nome,prodotto.descrizione,prodotto.prezzo,carrello.id FROM carrello inner join prodotto on carrello.cod_p=prodotto.id where carrello.user='".$_SESSION['user']."' ";
                    $result=$conn->query($sql);
                    if($result->num_rows > 0){
                        echo '<form action="elimina.php" method="post">';
                        echo '<table border="1px">';
                        echo'<tr><td>nome</td><td>quantità disponibile</td><td>prezzo unitario</td></tr>';
